...
class PILDicom_scaled(PILDicom):
    ...
    def create(cls, fn: (Path, str, bytes), mode=None) -> None:
        ...
        if isinstance(fn, (Path, str)):
            im = Image.fromarray(255 * dcmread_scale(fn).numpy())
        im.load()
        im = im._new(im.im)
...
class DCMPreprocessDataset(Dataset):
        ...
        if self.padding_to_square or self.resize is not None:
            ...
            if self.padding_to_square:
                ...
                padding = (
                    (pad_to - y) // 2,
                    (pad_to - x) // 2,
                    (pad_to - y) // 2 + extra_width,
                    (pad_to - x) // 2 + extra_height,
                )

                transform += [
                    tfms.Pad(padding=padding, fill=0, padding_mode="constant")
                ]
    ...
    def get_dcm_by_name(self, img_name):
        ...
        return (
            self.fnames.map(lambda x: x if x.name == img_name else None)
            .filter(None)[0]
            .dcmread()
        )
    ...
    def save(
        ...
        for idx in tqdm(range(len(self)), desc="Saving Images: "):
            ...
            data.save(
                filepath,
                format=extension,
                compress_level=0 if extension.lower() == "png" else None,
            )
